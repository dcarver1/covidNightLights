...
downloadFiles <- function(path1, location){
  ...
  write.csv(files, paste0("D:/geoSpatialCentroid/covidNightLights/data/filePaths/",location,"CSVS.csv"))
  
  # read in data 
  fileNames <- read.csv(paste0("D:/geoSpatialCentroid/covidNightLights/data/filePaths/",location,"CSVS.csv"))
  # select specific files 
  f1 <- fileNames[grep(pattern = "licorr.csv", x = fileNames$x),]
  
  baseDir <- paste0("D:/geoSpatialCentroid/covidNightLights/data/dailyData/",location,"/")
...
paths <- c("usa_sandiego")

locations <- c("sanDiego")
