...
# filter county to harris
harris <- con %>%
  dplyr::filter(STATEFP == 48 & NAME == "Harris")

# with a single dataset 
d1 <- read.csv("F:/nrelD/geoSpatialCentroid/covidNightLights/data/houstonDaily/usa_houston_1.licorr.csv")

## determine locations that are present within harris county 
...
sp2 <- st_as_sf(sp1)
sp2$inHarris <-  as.integer(st_intersects(sp2, harris))

## points of interest 
t3 <- sp2 %>%
  dplyr::filter(!is.na(inHarris))
ids <- unique(t3$ptid)
...
#export CSV
write.csv(x = df, file = "F:/nrelD/geoSpatialCentroid/covidNightLights/data/houstonDaily/harrisCountyDailySummary.csv")
